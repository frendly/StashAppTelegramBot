    ...
    async def get_random_image(
        ...
        try:
            ...
            if not all_galleries:
                ...
                excluded_galleries = set(
                    filtering_lists.get("blacklisted_galleries", [])
                )
                ...
                if selected_gallery_id:
                    ...
                    if update_last_selected:
                        try:
                            self.database.update_gallery_last_selected(
                                selected_gallery_id
                            )
                    ...
                    image = (
                        await self.stash_client.get_random_image_from_gallery_weighted(
                    ...
                    if image:
                        logger.debug(
                            f"Изображение получено из галереи {selected_gallery_id} (взвешенный выбор с учетом всех галерей, просмотренности и свежести)"
                        )
                        return image
                    else:
                        logger.warning(
                            f"🔄 Fallback level 1: Не удалось получить изображение из галереи {selected_gallery_id} (все категории пусты), используем старый метод с фильтрацией"
                        )
                else:
                    logger.warning(
                        "🔄 Fallback level 1: Не удалось выбрать галерею взвешенным выбором, используем старый метод с фильтрацией"
                    )
        except Exception as e:
            logger.warning(
                f"🔄 Fallback level 1: Ошибка при взвешенном выборе галереи: {e}, используем старый метод с фильтрацией",
                exc_info=True,
            )
        ...
        try:
            ...
            if image:
                ...
                logger.warning(
                    "🔄 Fallback level 2: Старый метод с фильтрацией не вернул изображение, используем базовый метод"
                )
        except Exception as e:
            logger.warning(
                f"🔄 Fallback level 2: Ошибка при fallback методе с фильтрацией: {e}, используем базовый метод",
                exc_info=True,
            )
