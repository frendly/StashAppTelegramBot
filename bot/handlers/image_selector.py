...
class ImageSelector:
    ...
    def __init__(
        self, stash_client: StashClient, database: Database, voting_manager=None
    ):
        """
    ...
    async def _select_gallery_weighted(
        self, exclude_ids: list[str], update_last_selected: bool
    ) -> StashImage | None:
        """
        Взвешенный выбор галереи и получение изображения из неё.
        ...
        if not all_galleries:
            logger.warning(
                "Не удалось получить список галерей из StashApp, используем старый метод"
            )
            return None
        ...
        # Получаем статистику по галереям
        ...
        # Выбираем галерею с учетом всех факторов
        ...
        if not selected_gallery_id:
            logger.warning(
                "🔄 Fallback level 1: Не удалось выбрать галерею взвешенным выбором"
            )
            return None
        ...
        if update_last_selected:
            try:
                ...
                logger.debug(
                    f"Не удалось обновить last_selected_at для галереи {selected_gallery_id}: {e}"
                )

        # Получаем случайное изображение из выбранной галереи
        image = await self.stash_client.get_random_image_from_gallery_weighted(
            gallery_id=selected_gallery_id, exclude_ids=exclude_ids
        )

        if image:
            logger.debug(
                f"Изображение получено из галереи {selected_gallery_id} (взвешенный выбор)"
            )
            return image

        logger.warning(
            f"🔄 Fallback level 1: Не удалось получить изображение из галереи {selected_gallery_id}"
        )
        return None

    async def _fallback_to_filtered_selection(
        self, exclude_ids: list[str]
    ) -> StashImage | None:
        """
        Fallback на старый метод с фильтрацией.
        ...
        if image:
            logger.info(
                "✅ Fallback level 1 успешен: изображение получено через старый метод с фильтрацией"
            )
            return image

        logger.warning(
            "🔄 Fallback level 2: Старый метод с фильтрацией не вернул изображение"
        )
        return None

    async def _fallback_to_basic_selection(
        self, exclude_ids: list[str]
    ) -> StashImage | None:
        """
        Fallback на базовый метод без фильтрации.
    ...
    async def get_random_image(
        ...
        if not self.voting_manager:
            return await self._fallback_to_basic_selection(exclude_ids)

        # Пытаемся использовать взвешенный выбор галереи
        try:
            image = await self._select_gallery_weighted(
            ...
            logger.warning(
                f"🔄 Fallback level 1: Ошибка при взвешенном выборе галереи: {e}",
                exc_info=True,
            )

        # Fallback level 1: используем старый метод с фильтрацией
        try:
            image = await self._fallback_to_filtered_selection(exclude_ids)
            if image:
                return image
        except Exception as e:
            logger.warning(
                f"🔄 Fallback level 2: Ошибка при fallback методе с фильтрацией: {e}",
                exc_info=True,
            )
