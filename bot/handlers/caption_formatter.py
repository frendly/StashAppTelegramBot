...
class CaptionFormatter:
    ...
    def format_caption(
        ...
        performer_names = (
            [p["name"] for p in image.performers] if image.performers else []
        )
        ...
        try:
            ...
            try:
                ...
                stars_string, rating_value = self.calculate_display_rating(
                ...
                if positive_votes + negative_votes > 0:
                    caption_parts.append(
                        f"Вес: {weight:.1f} | {stars_string} ({rating_value}/5.0)"
                    )
                else:
                ...
                logger.warning(
                    f"Ошибка при получении веса/рейтинга для галереи {image.gallery_id}: {e}"
                )
            ...
            logger.warning(
                f"Ошибка при форматировании подписи для галереи {image.gallery_id}: {e}"
            )
    ...
    def format_threshold_caption(
        ...
        caption_parts.append(
            f'• Получили "-": {negative_votes} ({negative_percentage:.0f}%)'
        )
