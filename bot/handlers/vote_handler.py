...
class VoteHandler:
    ...
    def __init__(
        ...
        last_command_time: Optional[Dict[int, float]] = None
    ):
        """
        ...
        try:
            ...
            if should_show_threshold and image.gallery_id and image.gallery_title:
                ...
                if gallery_stats:
                    ...
                    new_caption = self.caption_formatter.format_threshold_caption(image, gallery_stats, is_preloaded_from_cache)
            ...
            if should_send_new_image:
                ...
                if self.photo_sender:
                    ...
                    logger.warning("photo_sender не инициализирован, не могу отправить новое фото")
                    await loading_msg.delete()
