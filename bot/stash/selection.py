...
COVERAGE_PENALTY_MULTIPLIER = (
    0.5  # Множитель штрафа за просмотренность (50% при 100% покрытии)
)
...
def select_gallery_by_weight(
    ...
    all_galleries: list[dict[str, Any]] | None = None,
    gallery_stats: dict[str, dict[str, Any]] | None = None,
    excluded_galleries: set | None = None,
) -> str | None:
    """
    ...
    if not all_galleries:
        ...
        for gallery_id, weight in weights_dict.items():
            ...
            if random_value <= accumulated_weight:
                logger.debug(
                    f"Выбрана галерея {gallery_id} с весом {weight:.3f} (random={random_value:.3f}, total={total_weight:.3f})"
                )
                return gallery_id

        last_gallery_id = list(weights_dict.keys())[-1]
        logger.warning(
            f"Floating point edge case: возвращаем последнюю галерею {last_gallery_id}"
        )
    ...
    for gallery in all_galleries:
        ...
        if base_weight <= 0:
            logger.debug(
                f"Пропускаем галерею {gallery_id} с неположительным весом {base_weight}"
            )
            continue
        ...
        if last_selected == 0:
            days_since = (
                MAX_DAYS_FOR_FRESHNESS + 1
            )  # Никогда не выбиралась - максимальный бонус
        else:
        ...
        if modified_weight <= 0:
            logger.debug(
                f"Пропускаем галерею {gallery_id} с нулевым/отрицательным модифицированным весом"
            )
            continue
