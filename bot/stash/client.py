...
class StashGraphQLClient:
    ...
    async def execute_query(
        ...
        if not self.session:
            raise RuntimeError(
                "HTTP сессия не инициализирована. Используйте async with."
            )
        ...
        try:
            async with self.session.post(
                self.api_url, json=payload, headers=self._get_headers(), auth=self.auth
            ) as response:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            f"⏱️  GraphQL query failed after {duration:.3f}s: HTTP {response.status}, non-JSON response: {text_response[:500]}"
                        )
                    except asyncio.CancelledError:
                        # Пробрасываем CancelledError дальше
                        raise
                        ...
                        logger.error(
                            f"⏱️  GraphQL query failed after {duration:.3f}s: HTTP {response.status}, failed to read response body"
                        )
                    ...
                    raise Exception(
                        f"GraphQL API вернул не-JSON ответ при статусе {response.status}"
                    )
                ...
                if response.status >= 400:
                    ...
                    if error_details:
                        ...
                        logger.error(
                            f"⏱️  GraphQL query failed after {duration:.3f}s: HTTP {response.status}, GraphQL error: {error_msg}"
                        )
                        logger.debug(f"Full error response: {data}")
                    else:
                        logger.error(
                            f"⏱️  GraphQL query failed after {duration:.3f}s: HTTP {response.status}, response: {data}"
                        )
                    ...
                    error_msg = error_details[0].get("message", "Unknown error")
                    logger.error(
                        f"⏱️  GraphQL query failed after {duration:.3f}s: GraphQL error: {error_msg}"
                    )
                    logger.debug(f"Full error response: {data}")
    ...
    async def download_image(self, image_url: str) -> bytes | None:
        ...
        try:
            ...
            async with self.session.get(download_url, auth=self.auth) as response:
                ...
                logger.info(
                    f"⏱️  Image download: {duration:.3f}s ({size_kb:.1f} KB, {size_kb / duration:.1f} KB/s)"
                )
                return image_data
