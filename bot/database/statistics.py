...
class StatisticsRepository:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount == 0:
                    logger.warning(
                        f"Галерея {gallery_id} не найдена для обновления количества изображений"
                    )
                    return False

                conn.commit()
                logger.debug(
                    f"Обновлено количество изображений для галереи {gallery_id}: {total_images}"
                )
                return True
        except Exception as e:
            logger.error(
                f"Ошибка при обновлении количества изображений для галереи {gallery_id}: {e}"
            )
            return False
    ...
    def get_galleries_needing_update(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return [{"gallery_id": row[0], "gallery_title": row[1]} for row in results]
    ...
    def should_update_image_count(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                ...
                logger.warning(
                    f"Ошибка при сравнении даты для галереи {gallery_id}: {e}"
                )
                return True
