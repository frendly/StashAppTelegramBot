    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if "file_id_high_quality" not in sent_photos_columns:
                cursor.execute(
                    "ALTER TABLE sent_photos ADD COLUMN file_id_high_quality TEXT"
                )
            ...
            if "weight" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN weight REAL DEFAULT 1.0"
                )
                logger.info("Добавлено поле weight в gallery_preferences")

            if "excluded" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN excluded BOOLEAN DEFAULT FALSE"
                )
                logger.info("Добавлено поле excluded в gallery_preferences")

            if "excluded_at" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN excluded_at TIMESTAMP"
                )
                logger.info("Добавлено поле excluded_at в gallery_preferences")

            if "threshold_notification_shown" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN threshold_notification_shown BOOLEAN DEFAULT FALSE"
                )
                logger.info(
                    "Добавлено поле threshold_notification_shown в gallery_preferences"
                )

            if "total_images" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN total_images INTEGER DEFAULT 0"
                )
                logger.info("Добавлено поле total_images в gallery_preferences")

            if "images_count_updated_at" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN images_count_updated_at TIMESTAMP"
                )
                logger.info(
                    "Добавлено поле images_count_updated_at в gallery_preferences"
                )

            if "last_selected_at" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE gallery_preferences ADD COLUMN last_selected_at TIMESTAMP"
                )
            ...
            cursor.execute(
                "UPDATE gallery_preferences SET weight = 1.0 WHERE weight IS NULL"
            )
