...
class WeightsRepository:
    ...
    def update_gallery_weight(self, gallery_id: str, vote: int) -> float:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.commit()
            logger.debug(
                f"Обновлен вес галереи {gallery_id}: {current_weight:.3f} -> {new_weight:.3f} (vote={vote})"
            )
                    ...
                    try:
                        ...
                        if "T" in str(last_selected_at):
                            dt = datetime.fromisoformat(
                                str(last_selected_at).replace("Z", "+00:00")
                            )
                        else:
                            dt = datetime.strptime(
                                str(last_selected_at), "%Y-%m-%d %H:%M:%S"
                            )
    ...
    def update_gallery_last_selected(self, gallery_id: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if cursor.rowcount == 0:
                ...
                logger.debug(
                    f"Галерея {gallery_id} не найдена в БД для обновления last_selected_at"
                )
            else:
                conn.commit()
                logger.debug(
                    f"Обновлено время последнего выбора для галереи {gallery_id}"
                )
