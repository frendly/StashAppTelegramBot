...
class VotingManager:
    ...
    def __init__(
        ...
        self._filtering_cache: dict[str, list[str]] | None = None
    ...
    def _calculate_gallery_rating(
        self, positive_votes: int, negative_votes: int
    ) -> int:
        """
    ...
    def get_filtering_lists(self) -> dict[str, list[str]]:
        ...
        if (
            self._filtering_cache is not None
            and (current_time - self._filtering_cache_time) < self.cache_ttl
        ):
            logger.debug(
                f"⏱️  Using cached filtering lists (age: {current_time - self._filtering_cache_time:.1f}s)"
            )
    ...
    def get_cached_gallery_weights(self) -> dict[str, float]:
        ...
        if (
            ...
            and (current_time - self._weights_cache_time) < self.cache_ttl
        ):
            logger.debug(
                f"⏱️  Using cached gallery weights (age: {current_time - self._weights_cache_time:.1f}s)"
            )
    ...
    def check_exclusion_threshold(self, gallery_id: str) -> tuple[bool, float]:
        ...
        try:
            ...
            logger.error(
                f"Ошибка при проверке порога исключения для галереи {gallery_id}: {e}",
                exc_info=True,
            )
            return (False, 0.0)
