...
- Фоновая задача предзагрузки: ночью каждые 5 минут (с 2:00 до 6:00) загружает до 1000 изображений high quality
- Автоматическая обработка rate limits Telegram API (RetryAfter)
- Защита от бана (проверка на блокировку, остановка при множественных ошибках)
- Прогресс-логирование каждые 100 изображений с ETA
...
3. **Предзагрузка в служебный канал** - ночью каждые 5 минут с 2:00 до 6:00 (если `cache_channel_id` настроен)
   - Загружает до 1000 изображений за раз
   - Автоматическая обработка rate limits (RetryAfter)
   - Защита от бана
...
- Оптимизирована предзагрузка изображений: ночное время, большие пакеты (до 1000), обработка rate limits
- Добавлена защита от бана и улучшенное логирование
